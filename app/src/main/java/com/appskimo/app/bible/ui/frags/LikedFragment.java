...
import com.appskimo.app.bible.ui.adapter.LikedRecyclerViewAdapter;
...
public class LikedFragment extends Fragment {
    ...
    @Bean LikedRecyclerViewAdapter recyclerViewAdapter;
    ...
    private final SwipeRefreshLayout.OnRefreshListener refreshListener = new SwipeRefreshLayout.OnRefreshListener() {
        ...
        public void onRefresh() {
            ...
            if (bibleService != null) {
                bibleService.retrieveFavor(new More(), new Callback<More>() {
                    @Override
    ...
    @Subscribe
    @UiThread
    public void onEvent(MoreFavor event) {
        if (bibleService != null && currentMore.isHasMore()) {
            bibleService.retrieveFavor(currentMore, new Callback<More>() {
                @Override
                public void onSuccess(More more) {
                    currentMore = more;
    ...
    public void onEvent(Like event) {
        if (event.from().isLike() && !event.isLiked()) {
            recyclerViewAdapter.remove(event.getContentUid());
        } else {
            load();
        }
    }
